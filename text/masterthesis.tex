...
\end{lstlisting}
Leider hat die parallele Analyse gegenüber der höheren Geschwindigkeit auch einen Nachteil. Manchmal werden pdb-Dateien von der CCI nicht korrekt geladen, wenn die Analyse auf mehrere Theads verteilt läuft. Dieses Problem äußert sich dadurch, dass beispielsweise Zeilenangaben zu Methodenlängen nicht erfolgreich ermittelt werden können. Im Usus.NET Cockpit-Fenster entstehen dadurch sogenannte Phantom-Änderungen der Methoden-Metriken, wenn die gleiche Solution zweimal hintereinander analysiert wird, ohne dass der Quellcode geändert wurde. Aus diesem Grund kann unter dem Menü-Eintrag \texttt{Tools / Options / Usus.NET} eingestellt werden, ob Usus.NET die Analyse sequentiell oder parallel durchführen soll. Generell scheint die CCI Schwierigkeiten mit dem Ressource Management der pdb-Dateien zu haben. Wenn Usus.NET die pdb-Dateien nutzt und anschließend sofort wieder über die CCI freigibt, kann Visual Studio diese Dateien erst nach einer kurzen zeitlichen Verzögerung wieder ersetzen.


\section{Fallbeispiel Andrena-Kurs}
Einmal messen RefactoringCodingDojo -> \ref{fig:andrenarefacbefore}
Einmal messen RefactoringCodingDojoSolution -> \ref{fig:andrenarefacafter}


Das zu lösende Problem in der Codebasis des Refactoring-Beispiels ist die \texttt{GetMailMessage}-Methode der Klasse \texttt{MailComposer}. Mit 19 logischen Zeilen und einer zyklomatischen Komplexität von 7, ist diese Methode ein absoluter Hotspot.
\begin{figure}[h]
	\centering
