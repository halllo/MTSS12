...
Anforderung Clean Code Unterstützung haben, User Stories wegen Clean Code Metriken, leider noch kein CRAP oder das andere.
...
\section{Andrena Softwarequalitätsindex}
\label{sec:andrenasqi}
...
\label{eq:sqifmk}
\end{equation}
Isis verwendet an dieser Stelle wieder eine Zweidrittelkonstante aus Tabelle \ref{tab:sqniveaufmkfactors}, um die Gewichtung des Korrekturfaktors zu regulieren. Die Größe des Systems ist ebenfalls von Bedeutung und wird in Form der Anzahl aller relevanten Codezeilen (RLOC) verwendet. Wie Usus.net diese Codezeilen ermitteln kann ist in Unter-Unterabschnitt \ref{subsubsec:rloc} beschrieben.
\begin{table}[htb]
\centering
\renewcommand{\arraystretch}{1.5}
\rowcolors{1}{}{shadethmcolor}
...
\end{lstlisting}
Die Variable \texttt{parameters} enthält eine \texttt{IParameterProvider}-Referenz, über die die beiden Methoden \texttt{SqNiveau} und \texttt{SqNiveauCorrection} als Extension Methods aufgerufen werden können. Die \texttt{Sqi}-Klasse enthält für jede SQI-Metrik je ein Objekt vom Typ \texttt{RelativeSize}, \texttt{Calibration}, \texttt{MiddleSize} und \texttt{CorrectionCalibration}. Diese Typen repräsentieren die Metrik-abhängigen Berechnungen und Gewichtungen aus Unterabschnitt \ref{subsec:sqiparams}. Mit diesen Berechnungen lassen sich das Softwarequalitätsniveau sowie der Monolithkorrekturfaktor jeder SQI-Metrik bestimmen. Dessen Produkt kann anschließend aufsummiert werden um den Softwarequalitätsindex zu erhalten.
\paragraph{}
...
\subsubsection{Relevante Code-Zeilen}
\label{subsubsec:rloc}
...
\subsubsection{Compiler-Warnungen}
