...
\section{NRefactory}
\textit{NRefactory} ist Teil der freien .NET-Entwicklungsumgebung SharpDevelop, die in der Mono-Welt benutzt wird. Mono ist eine Implementierung des .NET-Frameworks für Windows, Linux, Mac OS und verschiedene mobile Plattformen. Fussnote bla bla \url{http://www.mono-project.com/}
...
Mit \textit{NRefactory} ist es möglich einen AST aus C$\#$-Quelltext zu erzeugen.
AST-Besucher können direkt als Implementierung des \texttt{IAstVisitor}-Interface oder als Ableitung von \texttt{DepthFirstAstVisitor} erstellt werden.
Mit einem solchen Besucher können strukturelle Code-Elemente wie if-Anweisungen sehr gut besucht werden.
Listing \ref{listing:nrefacastsample} zeigt Methoden, die in \texttt{IAstVisitor} definiert sind.
...
CC kann wie in \ref{subsubsec:cyclomaticcomplexity} beschrieben berechnet werden indem alle Entscheidungsstellen besucht und aufsummiert werden.
Methodenlänge kann entweder berechnet werden, indem alle Statements oder alle Zeilenumbrüche besucht und aufsummiert werden.

Allerdings kann durch die alleinige Erzeugung des AST der Abhängigkeitsgraph nicht erstellt werden. 

\subsection{Mono.Cecil}
\textit{NRefactory} ordnet Methodenaufrufe in Form von \texttt{InvocationExpression} erst dann der definierenden Stelle zu, wenn alle Typen und Methoden im AST mit einem \texttt{CSharpAstResolver} komplett aufgelöst werden. Dazu verwendet \textit{NRefactory} die \textit{Mono.Cecil}-Bibliothek, die Jean-Baptiste Evain als API zum Erzeugen, Analysieren und Verändern von .NET-Assemblies beschreibt \cite{MonoCecil}. \textit{Mono.Cecil} ist damit das CCI der Mono-Umgebung. \textit{NRefactory} nutzt \textit{Mono.Cecil} um Assemblies, auf dessen Typen der Code zugreift, zu analysieren und Methodenaufrufe den definierenden Typen zuzordnen.

Ähnlich CCI, also CCI Berechnung auch anhand von Operation Codes möglich

AST Manipulation möglich und Code-Erzeugung
