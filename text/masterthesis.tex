...
Wie die Verteilungen interpretiert werden können wird in Abschnitt \ref{sec:histogramanalysis} beschrieben.
...
\paragraph{}
Usus.net berechnet das $\lambda$ nach Formel \ref{eq:edistributionml}. Die Komponenten, die für diese Berechnung zuständig sind, wurden bereits in Abschnitt \ref{sec:distributions} vorgestellt. Das Ergebnis kann anschließend in der Histogramm-Oberfläche aus Unterabschnitt \ref{subsec:ususnethostigram} angezeigt werden. Abbildung \ref{fig:gdistususnet} zeigt beispielsweise die Verteilung der zyklomatischen Komplexität einer Beispielanwendung.
\begin{figure}[h]
	\centering
		\includegraphics[width=14cm]{images/gdistususnet.png}
	\caption{Usus.net Histogramm-Fenster mit $\lambda$ der geometrischen Verteilung}
	\label{fig:gdistususnet}
\end{figure}
In Kapitel \ref{chap:inaction} werden konkretere Beispiele betrachtet und mit Usus.net analysiert.
