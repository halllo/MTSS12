...
FxCop leider nicht geeignet, da Runner-Anwendung benötigt wird und Kommunikation zwischen Regel und Programm eingeschränkt ist.

CCI sehr geeignet, bis auf auf die Tatsache, dass Quellcode nicht analysiert werden kann und immer eine Assembly verwendet werden muss.

NRefactory geeignet, da Unterstützung für die Analyse von Quelltext verfügbar (zumindest für VB.net) dadurch allerdings die Methodenaufrufe keinem Ziel zugeordndet sind. Dafür muss auch NRefactory kompillieren, was dann nur in einer vollständigen Umgebung möglich ist.

Roslyn leider nicht geeignet, da es noch als CTP verfügbar ist und die Installation der Roslyn Umgebung erfordert. Langfristig die Lösung da wesentlich bessere Möglichkeiten als CCI und NRefactory zusammen (z.B. Workspace API). Langfristig die Lösung aller Code-Basierten-Programme.


Da leider kein Roslyn, CCI für alles kompilierte und für den Fall, das Analyse am Code benötigt wird zusätzlich NRefactory.
