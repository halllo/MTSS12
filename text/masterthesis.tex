...
wenig Zeit, bla bla
\\
viele Dinge mussten weggelassen werden, und können im Anschluss an diese Master-Thesis implementiert werden
\\
nur eine Evaluierung der \gls{SCA} möglich, weitere Technologie-Evaluierungen wie für die Graphenvisualisierung konnte nicht gemacht werden
\\
Mehr Settings erforderlich: Möglichkeit die \gls{SCA} ein oder auszuschalten. Manuelles Starten der \gls{SCA} auf Knopfdruck ermöglichen
\\
In der Evaluierung hat sich gezeigt, das die grafische Darstellung des Namespace-Graph insbesondere der Zyklen doch sinvoll ist
\\
Testabdeckung für \gls{SQI} automatisch ermitteln
\\
\textcolor[rgb]{1,0,0}{todo}
weitere coole sachen könnten anschließend ebenfalls realisiert werden
\\
definierte und erreichbare Ziele erlauben keinen Rahmen für Ausprobieren von coolen Sachen, da die verfügbare Zeit lediglich für die Implementierung eines minimalen Funktionsumfang genutzt werden kann
\\
Kleinster Zyklus in einem großen Zyklus hilft, die Problemstelle schneller zu finden
\\
die CRAP-\gls{Metrik} bestimmen um einen weiteren \gls{CleanCode}-Indikator zu haben
\\
mehr Verteilungen und deren Vergleich
\\
Hinweise zu sinnvollen Refactorings um eine gewünschte Verteilung zu erreichen
\\
