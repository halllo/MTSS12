...
Die \textit{Common Compiler Infrastructure} (CCI) ist eine Sammlung von Bibliotheken, die Kompiler-ähnliche Funktionen erfüllen.
Von Microsoft Research \cite{CCI}
CCI Metadata \cite{CCIMetadata}
CCI Code und AST Komponenten \cite{CCICode}

Die Assembly Microsoft.Cci.dll wird unter anderem mit dem Code Metrics Power Tool  aus \ref{subsec:vscodemetricspowertool} und FxCop \ref{sec:fxcop} genutzt.

Microsoft.Cci.dll entspricht allerdings nicht der Version, die von Codeplex bezogen werden kann. Die CCI besitzt viele Funktionen, die nicht öffentlich sondern nur intern aufrufbar sind. Die Funktionen, die konkrete Methoden-Metriken berechnen, sind nur von Programmen aufrufbar, die in der CCI-Assembly aufgelistet sind, wie in Abbildung \ref{fig:cciinternalsvisible}.
