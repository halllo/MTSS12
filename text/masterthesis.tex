...
...
\end{lstlisting}
Alle Schwellwertfunktionen sind Abbildungen von einem Objekt vom Typ \texttt{CommonReportKnowledge} auf eine Ganzzahl. \texttt{CommonReportKnowledge} ist Teil des Objektmodells des vollständigen Metrikberichts und wurde bereits in Abbildung \ref{fig:metricsreport} gezeigt. Dieses Objekt enthält einige allgemeine Daten der Metrik-Analyse wie beispielsweise die Anzahl der analysierten Klassen und Namespaces.
...
...
\end{lstlisting}
Die Variable \texttt{Metrics} enthält eine Referenz auf ein \texttt{MetricsReport}-Objekt. Das Histogramm wird in Form eines Objekts der Klasse \texttt{Distribution} erzeugt. Diese Klasse implementiert das Interface \texttt{IHistogram}. Über den Konstruktor kann eine Sequenz von \texttt{int}-Werten übergeben werden, aus denen die Verteilung erstellt wird. Ein Histogramm ist ein einfaches Schaubild, das auf der x-Achse die diskreten Werte einer Sequenz enthält. Auf der y-Achse wird die Häufigkeit des Auftretens des entsprechenden x-Werts abgebildet. Dadurch wird ersichtlich, wie oft ein bestimmter Wert in der Sequenz enthalten ist. Die \texttt{Distribution}-Klasse verwendet Funktionalität der öffentlichen Bibliothek Math.NET\footnote[1]{Math.NET Numerics "`numerical library"' \url{http://mathnetnumerics.codeplex.com/}} um das Histogramm zu erzeugen, wie in Listing \ref{listing:distributioncreation} gezeigt.
...
\end{lstlisting}
Die Variable \texttt{data} enthält die erwähnte Sequenz von Ganzzahlen (\texttt{IEnumerable<int>}), die über den Konstruktor übergeben wird. Usus.net unterstützt nur Histogramme mit positiven Werten. Ein fertiges Histogramm liegt in Form eines \texttt{IHistogram}-Objekts vor.  Die Werte der x-Achse können mithilfe der Eigenschaft \texttt{BinCount} auf 0 bis \texttt{BinCount}-1 bestimmt werden. Um den y-Wert zu einem x-Wert zu bestimmen, kann die Methode \texttt{ElementsInBin(x)} mit dem x-Wert als Parameter aufgerufen werden.
...
Abbildung \ref{fig:architecture2} zeigt die Verwendung der in Kapitel \ref{chap:ususnet} beschrieben Funktionen von \texttt{Usus.net.Core}. \texttt{Usus.net.Core} steht als Bibliothek in Form einer Assembly zur Verfügung und wird von der Bibliothek \texttt{Usus.net.View} genutzt. Diese Bibliothek enthält alle grafischen Oberflächen von Usus.net und wird von der \texttt{Usus.net} Visual Studio Erweiterung verwendet. Die Erweiterung stellt dann nur noch die Fenster-Infrastruktur für die Usus.net Oberflächen zur Verfügung. Usus.net ist kein Addin im Sinne von Unterabschnitt \ref{subsec:visualstudio}, sondern eine Erweiterung. In diesem Kapitel werden zunächst die Steuerelemente von \texttt{Usus.net.View} vorgestellt. Anschließend wird auf die Integration mit Visual Studio eingegangen.
...
\end{figure}
Dieses Fenster verhält sich wie alle anderen Fenster und kann in Visual Studio beliebig angedockt werden. Das ViewModel reagiert auf einen verfügbaren Metrikbericht, indem es die Einträge der Tabelle durch neue Einträge für die Statistiken ersetzt. Als Tabelle wird ein \texttt{DataGrid}-Objekt verwendet.
...
\end{lstlisting}
Nachdem das Paket des Fensters in Zeile 2 geladen wurde, kann das Fenster in Visual Studio geöffnet werden. Dazu verwendet Usus.net ein Objekt vom Typ \texttt{GlobalEventManager}. Sobald das Paket eines Fensters geladen wird, registriert dieses Paket eine Ereignisbehandlung bei der einzigen Instanz der \texttt{GlobalEventManager}-Klasse. Die Ereignisbehandlung kann mit der GUID des \emph{VSPackage} ausgeführt werden, was in Lisiting \ref{listing:ususnetdisplaywindows} in Zeile 3 zu sehen ist. Listing \ref{listing:ususnetdisplaywindowsevent} zeigt exemplarisch wie das Paket des Usus.net Cockpit-Fenster die Ereignisbehandlung zum Anzeigen des Fensters registriert.
...
\end{lstlisting}
Alle \emph{VSPackage}-Projekte, die zur der einen VSIX-Datei zusammengefasst werden teilen sich also die gleiche Instanz des \texttt{GlobalEventManager}-Objekts. Dadurch können Fenster- und Menüübergreifende Ereignisse definiert und ausgeführt werden. Dies ist die einzige Möglichkeit der Inter-Fenster-Menü-Kommunikation im Kontext von Usus.net.

\subsection{Zeilensprünge}
\label{subsec:vsrowjumps}
Listing \ref{listing:openfilejumprow} zeigt, wie Visual Studio mithilfe des \texttt{DTE2}-Objekts eine Datei öffnen und zu einer Zeile in dieser Datei springen kann.
...
\end{lstlisting}
Die Variable \texttt{fileName} enthält den vollständigen Pfad der zu öffnenden Datei und die Variable \texttt{fileLine} die Zeilennummer der zu markierenden Zeile.
\textcolor[rgb]{1,0,0}{todo}
...
\verb/^\"((\w|\s)+)\"$/
\begin{lstlisting}[caption={Zu der Methode eines \texttt{MethodMetricsReport}-Objekt springen},label={listing:methodmatching}]
"^{0}{1} {1}\\((((out |ref |this |params )?{1} {2})?(,[ ]?(out |ref |params )?{1} {2})*)\\)$"
"(private |protected |public |internal )?(static |virtual new |virtual |new |override |override sealed )?"
"([_a-zA-Z]+[\\._a-zA-Z0-9\\[\\]]*(<.*>)?)"
"([_a-zA-Z]+[_a-zA-Z0-9]*([ ]?=[ ]?[^,<> ]*)?)"
\end{lstlisting}
