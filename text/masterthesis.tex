...
Im Rahmen dieser Master-Thesis wird eine Visual Studio-Erweiterung entwickelt, die direktes Entwickler-Feedback anhand von \glslink{Metrik}{Softwaremetriken} gibt. Die Arbeit wird bei der andrena objects ag durchgeführt. Dabei dient das Plugin Usus für Eclipse als Vorlage. Die erstellte Erweiterung führt eine \glslink{SCA}{statische Code-Analyse} von .NET-Projekten in Visual Stuido 2010 durch und berechnet \glslink{Metrik}{Codemetriken}, die für andrena relevant sind. Um diese Metriken zu berechnen, analysiert das Tool die \glslink{ASM}{Assemblies}, die der Compiler bei jedem Kompiliervorgang erzeugt. Für die \glslink{SCA}{Analyse} wird die \gls{CCI} verwendet. Durch die Integration in Visual Studio können die Ergebnisse direkt in Fenstern in der IDE angezeigt werden.
\newline
Die entwickelte Erweiterung ist in der Lage, Softwareentwickler aktiv bei der Entwicklung von \gls{CleanCode} zu unterstützen. \gls{CleanCode} wurde von Robert C. Martin in seinem Buch "`\gls{CleanCode}"' vorgestellt und ermöglicht es, Quellcode besser verstehen und warten zu können. Über automatisiertes Feedback wird der Entwickler über Problemfälle in seinem Code informiert und kann diese schnell lokalisieren. Um zu entscheiden, ob es sich bei dem Code um \gls{CleanCode} handelt, nutzt die Erweiterung die Verteilungen der \glslink{Metrik}{Metriken}. Diese Verteilung wird analysiert und das Ergebnis als \gls{CleanCode}-\gls{Metrik} visualisiert. Zusätzlich kann das Tool den andrena-\gls{SQI} berechnen und dessen Veränderung über die Zeit protokollieren, sodass auch hier die Interpretation der inneren Qualität der Software ermöglicht wird.
\newline
Um das Tool und dessen Möglichkeiten der Entwicklerunterstützung zu evaluieren, wird eine Beispielaufgabe aus einem andrena-Kurs zum Thema Refactoring analysiert und die Veränderung in den \glslink{Metrik}{Metriken} entsprechend erfasst und dokumentiert. Außerdem werden verschiedene Codebasen mit dem Werkzeug analysiert, um die Geschwindigkeit zu messen und eventuelle Laufzeitprobleme der \glslink{SCA}{statischen Code-Analyse} von großen Projekten zu erkennen.
\newpage



\chapter*{Abstract}
During this master thesis a Visual Studio extension that is capable of providing direct development feedback based on \glslink{Metrik}{software metrics}, is developed. The extension is developed at andrena objects ag. The Eclipse plugin Usus, is going to be used for orientation. The developed extension shall be able to perform \glslink{SCA}{static code analysis} of .NET projects in Visual Studio 2010 in order to calculate the \glslink{Metrik}{code metrics} that are relevant for andrena. In order to calculate these \glslink{Metrik}{metrics} the \gls{CCI} is used. Because of the integration into Visual Studio the results can be displayed directly in the dialogs of the IDE.
\newline
The developed extension is capable of actively supporting developers to write \glslink{CleanCode}{clean code}. \glslink{CleanCode}{Clean code} was introduced by Robert C. Martins book "`\gls{CleanCode}"' and supports readability and maintainability of source code. The automated feedback informs the developer about problematic areas in the code and allows for quick pinpointing. To detect, whether the code is \glslink{CleanCode}{clean code}, the extension utilizes the distribution of the \glslink{Metrik}{metrics}. These distributions are analyzed and the result is visualized as a \glslink{CleanCode}{clean code} \glslink{Metrik}{metric}. The tool can additionally calculate the Software Quality Index of andrena, so that an interpretation of the inner software quality is possible.
\newline
The tool and its possibilities to support the developer are evaluated by analyzing an exercise of an andrena course about refactoring. Changes in the \glslink{Metrik}{metrics} are recognized and documented. In order to see how well the extension performs on big projects, the \glslink{SCA}{static code analysis} of a few projects is timed.
\newpage
...
\emph{Automatisiertes Testen} ist eine der wichtigsten Praktiken in der \gls{CleanCode}-Bewegung. Laut Robert C. Martin ist es die Aufgabe eines Entwicklers, keinen Schaden in einem Softwaresystem anzurichten. Damit meint er Schaden an der Funktionalität der Anwendung (äußere Qualität) und Schaden an der Struktur und Form des Quelltext (innere Qualität). Beides kann mit automatisierten Testfällen sichergestellt werden.
\end{definition}
\begin{definition}[Ständiges Refactoring]
\emph{Ständiges Refactoring} ist der effektivste Schutz gegen Schäden an der Struktur und damit der inneren Qualität eines Softwaresystems, welches evolutionär gewachsen ist und viele Anpassungen erfahren hat. Durch Tests kann dabei sichergestellt werden, dass Refactorings den Funktionsumfang und damit die äußere Qualität nicht beeinträchtigen. Martin Fowler beschreibt solche Refactorings in seinem Buch \cite{Refactoring}.
\end{definition}
...
\emph{Klasse} kommt aus dem lateinischen von \emph{classis} und bedeutet "`Aufgebot"'. Damit ist laut Oestereich eine Menge von Objekten der gleichen Struktur und des gleichen Verhaltens gemeint. Eine Klasse ist folglich auch der Typ aller seiner Objekte. Klassen können auch abstrakt sein. Eine abstrakte Klasse, oder auch Schnittstelle genannt, kann nicht als Objekt erzeugt werden. Eine nicht abstrakte Klasse, also eine konkrete Klasse, muss von dieser Klasse eine Vererbung der Struktur und des Verhaltens durchführen. Von dieser erbenden und konkreten Klasse kann dann ein Objekt erzeugt werden, das sowohl vom Typ der konkreten Klasse als auch vom Typ der abstrakten Klasse ist.
\end{definition}
\begin{definition}[Objekt]
\emph{Objekt} kommt ebenfalls aus dem lateinischen von \emph{obicere} und bedeutet "`entgegenhalten"'. Bernd Oestereich beschreibt es als "`Gegenstand der Erkenntnis und Wahrnehmung, des Denkens und Handelns"' und bezieht sich dabei auf das Brockhaus Lexikon. Ein Objekt ist eine Instanz einer Klasse.
\end{definition}
\begin{definition}[Attribut]
\emph{Attribut} kommt auch aus dem lateinischen von \emph{attributum} und bedeutet "`das Beigefügte"', was einer Eigenschaft oder einem Kennzeichen einer Sache entspricht. Die Daten, die ein Objekt ausmachen, werden anhand von Attributen gespeichert, auf die von den Operationen des Objekts aus zugegriffen werden kann. Attribute werden auch als Felder bezeichnet und lassen sich in Klassenfelder und Instanzfelder unterscheiden. Klassenfelder können von allen Objekten der Klasse gemeinsam genutzt werden, während Instanzfelder unterschiedliche Werte für konkrete Objekte haben können.
\end{definition}
...
\emph{Paket} wird von Oestereich als eine Ansammlung von Modellelementen bezeichnet. Dabei sind Modellelemente Klassen oder andere Pakete und dienen der besseren Strukturierung des Systems. Pakete werden auch als Namespaces bezeichnet, da sie eine benannte Zusammenfassung von Klassen und anderen Namespaces sind.
\end{definition}
Zusätzlich beschreibt Oerstereich zwei weitere Mittel von objektorientierten Softwaresystemen, die der Abstraktion dienen und die es erlauben, Verantwortlichkeiten einfacher zu organisieren. Beide Konzepte werden jetzt ebenfalls definiert.
...
\emph{Vererbung} entspricht einer \emph{Ist-ein-Beziehung} und gibt an, dass eine Klasse das Verhalten und die Struktur einer anderen Klasse erbt oder spezialisiert. Ein Objekt der erbenden Klasse ist damit auch ein Objekt der gerbten Klasse und hat somit auch eine Abhängigkeit von ihr.
\end{definition}
Mit diesen sieben Konzepten kann ein objektorientiertes Softwaresystem vollständig beschrieben werden.
...
In diesem Abschnitt sollen die Möglichkeiten beschrieben werden, die Visual Studio von Haus aus bietet um Software zu analysieren. Ab Visual Studio 2010 Premium können zu jeder Solution (Menge von Projekten) verschiedene \glslink{Metrik}{Codemetriken} direkt berechnet werden. Die Ergebnis-Anzeige ist in Abbildung \ref{fig:vsmetricsresult} dargestellt.
\begin{figure}[h]
	\centering
...
\end{figure}
Leider sind diese Funktion zur \gls{Metrik}-Berechnung und zur Visualisierung in der am weitesten verbreiten Professional-Version von Visual Studio nicht verfügbar. Zwei kostenlose Programme, die das Bestimmen der gleichen Menge von \glslink{Metrik}{Codemetriken} erlauben werden in den folgenden Unterabschnitten vorgestellt.
