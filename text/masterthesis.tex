...
\begin{description}
\item[Class Size] Anzahl statischer und nicht-statischer Methoden sowie Konstruktoren unabhängig von der Sichtbarkeit. Eine Klasse wird von Usus dann als Hotspot betrachtet, wenn der Wert der Klassengröße 12 übersteigt.
\item[Cumulative Component Dependency (of class)] Anzahl Klassen, von denen die betrachtete Klasse direkt und indirekt abhängt. Damit ist nicht der CCD eines Untersystems gemeint, der die CCD-Werte aller beinhalteten Klassen aufsummiert. Eine Klasse wird von Usus dann als Hotspot betrachtet, wenn ihr CCD-Wert über einer Schwelle liegt, die Projektgröße-abhängige ist. Bei kleinen Projekten liegt diese Schwelle bei 15\% der Klassenanzahl, während bei großen Projekten 5\% der Klassenanzahl empfohlen wird. Tatsächlich verwendet Usus die Schwelle von Klassen-CCD 4.
\end{description}
\subsection{Projektübergreifend}
\begin{description} 
\item[Average Component Dependency] Die ACD-Metrik des Systems entspricht dem Mittelwert der CCD-Werte aller Klassen. Die Prozentangabe in Usus entspricht dem ACD-Wert auf eine Klasse bezogen, also indem Usus noch einmal durch die Anzahl der Klassen dividiert.
\item[Class Size] Mittelwert über alle gewichteten Klassengrößen. Die Gewichtung wird von Usus anhand der Formel f(classsize) = (classsize / 12) - 1 vorgenommen.
