...
Im Namespace \texttt{andrena.Usus.net.Core.Math} existiert die Klasse \texttt{Histograms}. In dieser Klassen sind mehrere Extensions Methods definiert, die Histogramme von Methoden- und Klassen-Metriken bestimmen können. Listing \ref{listing:methodtypedistributions} zeigt, wie diese Methoden verwendet werden können.
\begin{lstlisting}[caption={Verteilungen von Methoden- und Klassen-Berichten erstellen},label={listing:methodtypedistributions}]
IHistogram css = Metrics.TypeHistogram(t => t.ClassSize);
IHistogram ccs = Metrics.MethodHistogram(m => m.CyclomaticComplexity);
\end{lstlisting}
Die Variable \texttt{Metrics} enthält eine Referenz auf ein \texttt{MetricsReport}-Objekt. Das Histogramm wird in Form eines Objekts der Klasse \texttt{Histogram} erzeugt. Diese Klasse implementiert das Interface \texttt{IHistogram} und bekommt über den Konstruktor eine Sequenz von \texttt{int}-Werten übergeben. Ein Histogramm ist ein einfaches Schaubild, das auf der x-Achse die diskreten Werte einer Sequenz enthält. Auf der y-Achse wird die Häufigkeit des Auftretens des entsprechenden x-Werts abgebildet. Dadurch wird ersichtlich, wie oft ein bestimmter Wert in der Sequenz enthalten ist. Die \texttt{Histogram}-Klasse verwendet Funktionalität der öffentlichen Bibliothek Math.NET\footnote[1]{Math.NET Numerics "`numerical library"' \url{http://mathnetnumerics.codeplex.com/}} um das Histogramm zu erzeugen. Diese Erzeugung ist in Listing \ref{listing:distributioncreation} dargestellt.
...
\paragraph{}
Usus.net nutzt Mechanismen um das Histogramm zu analysieren. Das Interface \texttt{IHistogram} definiert das Property \texttt{Fitting}, welches ein \texttt{FittingReport}-Objekt zurückgibt. Wenn ein Histogramm erzeugt wird, wird auch immer dieser Bericht erstellt. Durch das \texttt{FittingReport}-Objekt lassen sich die Informationen bestimmen, die über die Verteilung der Metriken Aufschluss geben können. So lässt sich beispielsweise mit dem \texttt{ForGeometricalDistribution}-Property der Parameter $\lambda$ der geometrischen Verteilung \ref{eq:gdistribution} bestimmen. Um die Parameter der Verteilungen zu bestimmen berechnet Usus.net statistische Kennzahlen (beispielsweise den Mittelwert) der Histogramm-Sequenz wieder mithilfe von Math.NET-Bibliothek. Wie diese Analyse genau begründet ist, wird in Abschnitt \ref{sec:histogramanalysis} beschrieben.
...
\end{equation}
Beide Funktionen sind von dem Parameter $\lambda$ abhängig. Die eigentliche Histogramm-Analyse besteht darin, diesen Parameter so zu bestimmen, dass die Funktionen das Histogramm möglichst genau beschreibt. Eine Methode das $\lambda$ zu berechnen ist die Maximum Likelyhood-Methode, die Weisstein ebenfalls in seinem MathWorld-Eintrag beschreibt \cite{MaximumLikelihood}. Mit dieser Methode kann eine Schätzer-Funktion errechnet werden, die in Abhängigkeit der Werte des Histogramms den Parameter $\lambda$ bestimmt. Sigbert Klinke, Patrick Lehmann, Bernd Rönz, Sibylle Schmerbach und Olga Zlatkin-Troitschanskaia haben den Schätzer für die Exponentialverteilung bereits bestimmt \cite{MLExponentialDist}. Den Schätzer der geometrischen Verteilung wurde von Künzer, Meister und Nebe berechnet \cite{MLGeometricsDist}. Beide Schätzer-Funktion sind identisch und sind in Formel \ref{eq:edistributionml} dargestellt.
\begin{equation}
...
\paragraph{}
Usus.net berechnet das $\lambda$ nach Formel \ref{eq:edistributionml}. Die Komponenten, die für diese Berechnung zuständig sind, wurden bereits in Abschnitt \ref{sec:distributions} vorgestellt. Das Ergebnis kann anschließend in der Histogramm-Oberfläche aus Unterabschnitt \ref{subsec:ususnethostigram} angezeigt werden. Abbildung \ref{fig:ususnet_histogram} zeigt beispielsweise die Verteilung der Klassengrößen einer Beispielanwendung. In Kapitel \ref{chap:inaction} werden konkretere Beispiele betrachtet und mit Usus.net analysiert.


\section{Andrena Software Qualitäts Index}
Theoretisch interpretieren Entwickler kleinere Metriken in der Regel als bessere Metriken, obwohl bessere Metriken nicht automatisch besseren Code beschreiben. In Bezug auf die Methodenlänge ist Robert C. Martin in seinem Buch sehr deutlich und verteidigt kurze Methoden \cite{CleanCode}. Ein Softwaresystem, welches evolutionär gewachsen ist, enthält leider nicht nur kurze Methoden sondern auch zunehmend längere. Eine Interpretation über die Verteilung wie in Abschnitt \ref{sec:histogramanalysis} kann dann sehr schnell an Aussagekraft verlieren. Die Gewichtung der Metriken, die in Usus für Eclipse und Usus.net vorgenommen und in Abschnitt \ref{sec:metrics} beschrieben wird, versucht die Interpretation trotzdem zu ermöglichen. Deswegen zeit das Cockpit-Fenster der beiden Usus-Versionen auch nicht die Rohmetriken an. Die Metrikgewichtung von Usus geht auf das andrena-interne Werkzeug Isis zurück. Isis erlaubt es ungewichtete Metriken, die von einem anderen Tool erstellt wurden, zu importieren und zu bewerten. Das Ergebnis ist der von andrena sogenannte "`Softwarequalitätsindex"', welcher in dem Manuskript von Dr. Eberhard Kuhn ausführlich beschrieben wird \cite{IsisSQI}.

\textcolor[rgb]{1,0,0}{todo}:

Mittlere Testabdeckung (TA)
Anzahl Packages in Zyklen (PC)
Cyclomatic Complexity (CC):  Anzahl Methoden, deren CC-Wert größer 5 ist
Average Component Dependency Class (ACD): gemittelter Wert über alle Klassen, wie viel Prozent aller Klassen von jeder Klasse direkt oder indirekt referenziert werden.
Anzahl Klassen größer 20 Methoden (AKM)
Anzahl Methoden größer 15 LOC (AMS)
Anzahl Compilerwarnungen (CW) gemäß Eclipse default Einstellung

\begin{center}
   \begin{tabular}{ l | l | l }
		Metrik & Relative Größe & Zweidrittelkonstante \\
		\hline
		Anzahl Packages in Zyklen & 1 / Anzahl Packages & 1/15 \\
		Anzahl Methoden CC größer 5 & 1 / Anzahl Methoden & 1/50 \\
		Mittlere Average Component Dependency & 1 & 100 * Anzahl KlassenHoch-1/3 \\
		Anzahl Klassen größer 20 Methoden & 1 / Anzahl Klassen & 1/25 \\
		Anzahl Methoden größer 15 Lines & 1 / Anzahl Methoden & 1/25 \\
		Anzahl Compilerwarnungen & 1 / Anzahl Klassen & 1/50 \\
	\end{tabular}
\end{center}

SQNiveau(Metrik) = 100 / (1,5 hoch( Relative Größe / Zweidrittelkonstante))
SQNiveau(Testabdeckung) = Testabdeckung

fmk = 1 / 1,5 hoch((Mittlere Größe / Zweidrittelkonstante)hoch3)
SQNiveau = SQNiveau * fmk

\begin{center}
   \begin{tabular}{ l | l | l }
		Metrik & Relative Größe & Zweidrittelkonstante \\
		\hline
		Anzahl Packages in Zyklen & RLOC / Anzahl Packages & 3000 \\
		Anzahl Methoden CC größer 5 & RLOC / Anzahl Methoden & 15 \\
		Average Component Dependency & RLOC / Anzahl Klassen & 200 \\
		Anzahl Klassen größer 20 Methoden & RLOC / Anzahl Klassen & 200 \\
		Anzahl Methoden größer 15 Lines & RLOC / Anzahl Methoden & 15 \\
	\end{tabular}
\end{center}

SQI = Summe (SQNineaui * Gewicht * 0.1)
...
\begin{center}
   \begin{tabular}{ l | l }
		...
		\hline
		Testabdeckung & 2.28 \\
		Anzahl Packages in Zyklen & 1.93 \\
		Anzahl Methoden CC größer 5 & 1.75 \\
		Average Component Dependency & 1.58 \\
		Anzahl Klassen größer 20 Methoden & 1.05 \\
		Anzahl Methoden größer 15 Lines & 1.05 \\
		Anzahl Compilerwarnungen & 0.36 \\
	\end{tabular}
\end{center}
