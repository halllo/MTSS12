...
\paragraph{}
Wer implementiert das \texttt{IParameterProvider}-Interface auf Seite des Metrik-Berichts?
Das ViewModel der SQI-View verwendet eine Klasse \texttt{SqiParameters}, die das Interface implementiert. Diese Klasse verwaltet \texttt{SqiParameter}-Objekte, die direkt an die Oberfläche gebunden werden können. An der Oberfläche werden alle SQI-Parameter als Tabelle (\texttt{DataGrid}) angezeigt, die editiert werden kann. Eine Änderung eines Werts wird durch das Data Binding des ViewModels direkt in den \texttt{SqiParameter}-Objekten vorgenommen. Anschließend kann der SQI neu berrechnet werden.
\textcolor[rgb]{1,0,0}{todo}
