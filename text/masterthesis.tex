...
\section{NRefactory}
\textit{NRefactory} ist Teil der freien .NET-Entwicklungsumgebung SharpDevelop, die in der Mono-Welt benutzt wird. Das Team hinter Mono beschreibt Mono als Implementierung des .NET-Frameworks für Windows, Linux, Mac OS und verschiedene mobile Plattformen \cite{Mono}.
...
Der AST kann verändert und anschließend neuer Code erzeugt werden.
...
\end{lstlisting}
Der Code orientiert sich an dem Demo Beispiel von Daniel Grunwald in dem \textit{NRefactory}-Repository \cite{NRefacRepo}. Das übergebene \texttt{CompilationUnit}-Objekt ist der Wurzelknoten eines \textit{NRefactory}-AST. Jede Assembly, die in die Auflösung mit einbezogen werden soll, wird durch einen in dieser Assembly enthaltenen Typ identifiziert. Die eigentliche Auflösung wird über einen Aufruf der Methode \texttt{CSharpAstResolver.ApplyNavigator} synchron durchgeführt, die ein Objekt vom Typ \texttt{IResolveVisitorNavigator} erwartet und diesem alle aufgelösten Details zu allen Knoten im AST mitteilt. Ähnlich wie die CCI erlaubt auch \textit{Mono.Cecil} das manuelle Einlesen von Assemblies und \textit{Program Database}-Dateien um diese anhand der CIL-Anweisungen zu analysieren und verändert zu speichern.
