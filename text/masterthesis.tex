...
\definecolor{shadethmcolor}{rgb}{.955,.955,.955}   % Farbe des Hintergrundes
...
Ziel der Arbeit ist die Entwicklung einer Visual Studio-Erweiterung (im Folgenden Usus.NET genannt), die es erleichtern soll, Clean Code zu entwickeln, indem die Entwickler Feedback anhand von \glslink{Metrik}{Softwaremetriken} und daraus abgeleiteten Qualitätskenngrößen erhalten. Die Arbeit wird bei der andrena objects ag durchgeführt. Dabei dient das Plugin Usus für Eclipse als Vorlage. Die erstellte Erweiterung führt eine \glslink{SCA}{statische Code-Analyse} von .NET-Projekten in Visual Studio 2010 durch und berechnet bestimmte, für andrena relevante \glslink{Metrik}{Codemetriken}. Um diese Metriken zu berechnen, analysiert das Tool die \glslink{ASM}{Assemblies}, die die verschiedenen .NET-Compiler bei jedem Kompiliervorgang erzeugen. Für die \glslink{SCA}{Analyse} wird die \gls{CCI} verwendet. Durch die Integration in Visual Studio können die Ergebnisse direkt in Fenstern in der IDE angezeigt werden.
\newline
Die entwickelte Erweiterung ist in der Lage, Softwareentwickler aktiv bei der Entwicklung von \gls{CleanCode} zu unterstützen. \gls{CleanCode} wurde von Robert C. Martin in seinem Buch "`\gls{CleanCode}"' vorgestellt und ermöglicht es, Quellcode besser verstehen und warten zu können. Über automatisiertes Feedback wird der Entwickler über Problemfälle in seinem Code informiert und kann diese schnell lokalisieren. Um zu entscheiden, ob es sich bei dem Code um \gls{CleanCode} handelt, nutzt die Erweiterung die statistischen Verteilungen der \glslink{Metrik}{Metriken}. Diese Verteilungen werden analysiert und das Ergebnis als \gls{CleanCode}-\gls{Metrik} visualisiert. Zusätzlich kann das Tool den andrena-\gls{SQI} berechnen und dessen Veränderung über die Zeit protokollieren, sodass auch hier die Interpretation der inneren Qualität der Software ermöglicht wird.
\newline
...
\chapter*{Abstract}
Task of this thesis is the development of a Visual Studio extension (hereafter referred to as Usus.NET) that is capable of providing direct development feedback based on \glslink{Metrik}{software metrics}. The extension is developed at andrena objects ag. The Eclipse plugin Usus is going to be used for orientation. The developed extension shall be able to perform \glslink{SCA}{static code analysis} of .NET projects in Visual Studio 2010. This implies, that several \glslink{Metrik}{code metrics}, which are relevant to andrena, have to be calculated using the \gls{CCI}. Because of the integration into Visual Studio the results can be displayed directly in the dialogs of the IDE.
\newline
The developed extension is capable of actively supporting developers to write \glslink{CleanCode}{clean code}. \glslink{CleanCode}{Clean code} was introduced by Robert C. Martins book "`\gls{CleanCode}"' and supports readability and maintainability of source code. The automated feedback allows the developer to localize problematic areas in the code. To detect whether the code is \glslink{CleanCode}{clean code}, the extension utilizes the statistical distributions of the \glslink{Metrik}{metrics}. These distributions are analyzed and the result is visualized as a \glslink{CleanCode}{clean code} \glslink{Metrik}{metric}. The tool can additionally calculate the Software Quality Index of andrena, so that an interpretation of the inner software quality is possible.
\newline
The tool and its possibilities to support the developer are evaluated by analyzing an exercise of an andrena course about refactoring, to verify that changes in the code are picked up by the extension and lead to changes in the different \glslink{Metrik}{metrics}. In order to see how well the extension performs on big projects, the \glslink{SCA}{static code analysis} of a few projects is timed.
\newpage
...
Entwickler mit wenig Erfahrung arbeiten an kleineren Codebasen und wollen auf einfache Weise einen Überblick über Problemfälle des Systems behalten. Mithilfe des Systems wollen sie diese rechtzeitig beseitigen und sicherstellen, dass dadurch keine neuen Probleme entstehen.
\end{stakeholder}
\begin{stakeholder}[.NET-Entwickler mit viel Erfahrung]
\label{stake:muchex}
Entwickler mit viel Erfahrung arbeiten an großen und komplizierten Codebasen und möchten das System einsetzen um Tendenzen im System zu erkennen und Spezialfälle zu analysieren. Auch sie wollen einen besseren Überblick erhalten und das System aus möglichst vielen verschiedenen Perspektiven sehen.
\end{stakeholder}
...
\end{figure}
Das in Abbildung \ref{fig:usus_histogram} gezeigte Histogramm-Fenster zeigt die Verteilung der \gls{Metrik} an, die zuvor über einen Einfachklick im Usus Cockpit markiert wurde. Es werden immer nur die Daten für eine \gls{Metrik} angezeigt. Die Ansicht kann vergrößert, verkleinert, skaliert und als Grafik gespeichert werden.
...
\end{equation}
wobei $Bs(G)$ die Menge aller binären Entscheidungen im Ablaufgraphen $G$ darstellt. Voraussetzung für diese Rechnung ist, dass die Methode nur einen Eingang und nur einen Ausgang besitzt. Der Quellcode in Listing \ref{listing:simpleifs} soll die Grundlage für eine demonstrative Berechnung der zyklomatischen Komplexität darstellen.
...
\paragraph{}
Listing \ref{listing:roslynsyntaxtreecreation} zeigt die Erzeugung eines Syntaxbaums aus einem Quellcodeausschnitt, der in der Variable \texttt{sourceText} enthalten ist. Ähnlich wie NRefactory bietet auch Roslyn die Möglichkeit, den Baum mit LINQ-Abfragen zu analysieren, da die Eigenschaft \texttt{DescendentNodes} ein \texttt{IEnumerable} zurückliefert. LINQ\footnote[1]{Mehr Informationen zu "`LINQ (Language-Integrated Query)"' \url{http://msdn.microsoft.com/de-de/library/bb397926.aspx}} ist ein Sprachkonstrukt, das es erlaubt, mit Keywords wie \texttt{from}, \texttt{where} und \texttt{select} SQL-ähnliche Abfragen direkt in C$^\sharp$ oder VB.NET zu schreiben.
...
\end{figure}
\paragraph{}
Zuerst wurde in Abschnitt \ref{sec:fxcop} FxCop betrachtet. Diese Technologie wird im Rahmen der Evaluierung als nicht tauglich bewertet. Der Grund dieser Entscheidung ist auf die Umgebung zurückzuführen. Die eigentliche \glslink{SCA}{statische Code-Analyse} könnte von einer eigenen speziellen Regel durchgeführt, die im Kontext der FxCop Runner-Anwendung ausgeführt wird. Dadurch wird die Kommunikation zwischen Regel und aufrufendem Programm erschwert was zu einer größeren Distanz zwischen Informationssammlung und Informationsaufbereitung führt.
\paragraph{}
